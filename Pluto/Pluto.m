...
+ (void)initializePluto {
    ...
    PltiPhone5_8 = PltScreenWidth == 375.0 && PltScreenHeight == 812.0;
    PltiPhone5_5 = PltScreenWidth == 414.0 && PltScreenHeight == 736.0;
    PltiPhone4_7 = PltScreenWidth == 375.0 && PltScreenHeight == 667.0;
    PltiPhone4_0 = PltScreenWidth == 320.0 && PltScreenHeight == 568.0;
    PltiPhone3_5 = PltScreenWidth == 320.0 && PltScreenHeight == 480.0;
    
    if (PltiPhone5_8) {
        PltStatusBarHeight = 44.0f;
    } else {
        PltStatusBarHeight = 20.0f;
    }
    PltNavigationBarHeight = 44.0f + PltStatusBarHeight;
    if (PltiPhone5_8) {
        PltTabBarHeight = 83.0f;    //iPhone X底部操作范围为34高度
    } else {
        PltTabBarHeight = 49.0f;
    }
...
+ (void)load {
    ...
    dispatch_once(&onceToken, ^{
        ...
        Method method2 = class_getInstanceMethod(self, @selector(plt_layoutSubviews));
        method_exchangeImplementations(method1, method2);
    });
}

- (void)plt_layoutSubviews {
    if (self.pltPlaceholderTextView && self.pltPlaceholderTextView.hidden == NO) {
        self.pltPlaceholderTextView.frame = self.bounds;
    }
    [self plt_layoutSubviews];
}
